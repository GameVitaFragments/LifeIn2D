...
void main() {
  ...
  col = texture2D(texture, uv - 0.0025*abs(InsSide)).rgb;
  col = texture2D(texture, uv + 0.0025*0.5*abs(InsSide)).rgb;

  
  col =mix(col,(vec3(1.0, 1.0, 1.0) - col),n);
  vec3 screenAlpha = vec3(texture2D(screenOverlay,uv).a);
  col -= screenAlpha;
  col = clamp(col,vec3(0),vec3(1));

  col += texture2D(screenOverlay,uv).xyz*screenAlpha; 
